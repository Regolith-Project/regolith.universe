...
def terrain_mesh_posts(surface_yx: np.ndarray, cfg: TerrainConfig, stride: int) -> np.ndarray:
    """Compute indices of the heightmap posts kept as mesh vertices, along one axis.
...
def _grid_lookup(xs: np.ndarray, ys: np.ndarray, z: np.ndarray):
    """elevation(x_m, y_m) over a regular grid of posts, evaluated on the TRIANGLES the mesh is built from - two per quad, split on the a->c diagonal (u == v).
